...
public class HomeGui extends VerticalLayout {
    ...
    Set<ChordApp> listOfSelectedChords;
    List<ChordApp> listOfAllChords;

    @Autowired
    public HomeGui(TrackRepo trackRepo, TrackController trackController, ChordRepo chordRepo,ChordController chordController) {
    ...
    private int saveInDatabase(TrackApp trackApp, Set<ChordApp> listOfSelectedChords) {
    ...
    public void initializeUI() {
        listOfSelectedChords = new HashSet<>();
    ...
    private Set<ChordApp> getListOfSelectedChords() {
    ...
    public void setTracks(){
        Optional<TrackApp> optionalTrackApp = trackRepo.findById(1L);
        TrackApp trackApp = optionalTrackApp.get();
            trackName.setValue(trackApp.getName());
            trackBody.setValue(trackApp.getBody());
        Set<TrackApp> chordApps = trackRepo.getAll();
            for (TrackApp chordApp: chordApps) {
                for (ChordApp chordApp1: chordApp.getChordApps()) {
                    chosenChords.add(chordApp1.getChordName() + ", ");
                }
            }
        }
